...
function NoSpotifyAccountWarning() {
    ...
    return (
        <Alert rounded={15} mb={10} flexDirection={"column"} justifyContent={'center'} alignItems={'center'}
            ...
            <AlertDescription maxWidth='sm'>
                Hello there! 👋 For your profile to be visible and shareable, you need to link your Spotify account!
            </AlertDescription>

            <Button onClick={() => router.push("/link")} my={5} size={'sm'} leftIcon={<FaSpotify/>}>
...
function StatusBadge({status}: { status: string }) {
    return (
        <Box w={2} h={2} bg={status === 'success' ? 'green.500' : status === 'loading' ? 'yellow.500' : 'red.500'}
             rounded={'full'}/>
    )
}
