...
async function handler(req: NextApiRequest, res: NextApiResponse<any>) {
  try {
    if (req.method === 'POST') {
      ...
      let {name, username, password, email, confirm_password} = validatedData;
      ...
      return new SuccessResponse('User created successfully').handleResponse(
        req,
        res,
      );
    }
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
      console.log(error);
      if (error.code === 'P2002') {
        return new ConflictErrorResponse('User already exists').handleResponse(
          req,
          res,
        );
      }
